...
FIRE_VARIANTS = ["lean", "coast", "barista", "fat"]
...
def render() -> None:
    ...
    try:
        ...
        with st.form("fire_goal_setup_form"):
            variant = st.selectbox("FIRE variant", FIRE_VARIANTS, index=FIRE_VARIANTS.index(profile.get("fire_variant")) if profile.get("fire_variant") in FIRE_VARIANTS else 0)
        ...
        st.caption(f"{variant.title()} FIRE with a spending band of ${spending_floor:,.0f} to ${spending_ceiling:,.0f}.")
    finally:
        conn.close()
